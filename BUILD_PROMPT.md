...
#
# Usage:
#   claude -p "$(cat BUILD_PROMPT.md)" --dangerously-skip-permissions
#
# Or interactively:
#   claude
#   > Read BUILD_PROMPT.md and execute it.
#
# Place these files in the working directory before running:
#   - cbc_copilot_project_structure.md  (the architecture document)
#   - CBC_Req_Validation_v1_3.xlsx      (requirements & process flow)
#   - 1_Architectural.pdf               (sample building plan for testing)
#   - Hager-Price-Book-18-Complete-Effective-2-2-26_compressed-1.pdf (product catalog)
...
3. **`1_Architectural.pdf`** — A real building plan (Dutch Bros Coffee, Alexandria LA). Use this as the test fixture. It contains door schedules, frame schedules, hardware sets, Division 10 specialties (restroom accessories, corner guards, signs, lockers), FRP wall panel details, and fire-rated openings. Parse it to understand the PDF structure you will need to handle.

4. **`Hager-Price-Book-18-Complete-Effective-2-2-26_compressed-1.pdf`** — The Hager price book (744 pages). Hager is 75% of CBC's volume. Use this to understand the price-book PDF structure: product part numbers, list prices, multipliers, adders (electrification, NRP hinges, premium finishes), and finish codes.

After reading all four files, output a brief **BUILD PLAN** (no more than 30 lines) summarizing:
...
Create the complete directory tree exactly as specified in `cbc_copilot_project_structure.md`. Every directory and file must exist. Create placeholder files where content will be filled in later phases. Specifically:
...
10. **`cost_sourcing_rules.md`** — Three paths: (1) P21 last-PO if sold within 1 year, no price increase (~90% correct); (2) list × multiplier for non-special items; (3) vendor RFQ for custom/never-sold/special-prep. Freshness rule: 6-8 months unreliable, 3-4 years discard. Distributor-bought lines (Banner, SecLock, Pionite, Wilsonart) need MANUAL price entry with "price may be out of date" refresh prompt.
...
7. **`development-description.md`** — System overview, build phases, stakeholder notes (Kevin, Rick, Shanna), long-term engagement model.
...
3. Copy `CBC_Req_Validation_v1_3.xlsx` to `docs/CBC_Req_Validation_v1_3.xlsx` as reference.
4. Copy `cbc_copilot_project_structure.md` to `docs/architecture_source.md`.
