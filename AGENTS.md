...
## Architecture layers
1. **Agents** (`.Codex/agents/`) — 10 sub-agents, one per process phase
2. **Skills** (`.Codex/skills/`) — 9 reusable task workflows with scripts & references
3. **Rules** (`.Codex/rules/`) — 8 project-scoped constraint files
4. **Guardrails** (`.Codex/hooks/`) — 5 executable hooks (PreToolUse / PostToolUse)
5. **Memory** (`.Codex/memory/`) — 13 persistent reference-data files
6. **MCP Servers** (`mcp-servers/`) — 5 tool providers (pdf-tools, catalog,
   calc-engine, artifact-storage, p21-connector),
   registered in **`.mcp.json`** at the repo root. `.Codex/settings.json` has no
...
The estimator drives the pipeline through a web app; Codex works behind it.
...
- **`apps/worker/`** — claims queued jobs and runs `Codex --print`, then syncs what
  Codex wrote on disk into MongoDB.
...
- **Containers** — `docker compose up -d --build` runs all five. `api` and
  `worker` share one image; it runs as a **non-root** user because Codex
...
- **Provider** — which Codex runs the passes is configured on the settings
...
written back down to `extracted/` and `priced/` so Codex's next pass reconciles
...
`.Codex/rules/scope-boundaries.md` — a project rule, so it is already in context.
...
See `.Codex/memory/manual_cutoff.md` and the accuracy-trust project rule.
